<template>
  <el-dialog title="参数编辑" :visible.sync="_visible" width="45%" @open="onDialogOpen" @closed="onDialogClosed"
    :close-on-click-modal="false">
    <el-form ref="form" :model="value_" label-width="100px">
      <el-form-item label="参数名称">
        <el-input v-model="value_.name" placeholder="请输入参数名称"></el-input>
      </el-form-item>
      <el-form-item label="参数类型">
        <el-radio-group v-model="value_.type">
          <el-radio-button label="ParamConfigNumber">数字</el-radio-button>
          <el-radio-button disabled label="ParamConfigLetter">字母</el-radio-button>
          <el-radio-button disabled label="ParamConfigTime">时间</el-radio-button>
          <el-radio-button disabled label="ParamConfigCustomize">自定义</el-radio-button>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="参数配置">
        <components :is="value_.type" :value="value_.paramConfig" @change="onParamConfigChange">
        </components>
      </el-form-item>
    </el-form>
    ...
    <span slot="footer">
      ...
      <el-button type="primary" @click="$emit('confirm',JSON.parse(JSON.stringify(value_)))">确 定</el-button>
    </span>
  </el-dialog>
</template>
